 ...
 * author : xxd
 * date   : 2021/9/2
 * desc   : 修改 LinePagerIndicator，用来适配设计的需求
 * 1. 增加指示器本身是渐变的颜色（不是滑动渐变）
 * 2. 增加指示器是一张图片的模式
 */
public class CustomLinePagerIndicator extends View implements IPagerIndicator {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        // 计算颜色
        if (mGradientColors != null && mGradientColors.size() > 0) {
            // 需要用到宽高在处理
        ...
        if (mGradientColors != null) {
            //如果只有一种颜色，就将这种颜色设置为指示器的背景色；如果大于一种，则从左往右设置为渐变色
            if (mGradientColors.size() == 1) {
                mPaint.setColor(mGradientColors.get(0));
            } else {
                int[] colors = new int[mGradientColors.size()];
                float[] mPosition = new float[mGradientColors.size()];
                for (int i = 0; i < mGradientColors.size(); i++) {
                    colors[i] = mGradientColors.get(i);
                    mPosition[i] = 1.0f / mGradientColors.size() * (i + 1);
                    if (i == 0) {
                        mPosition[i] = 0;
                    } else if (i == (mGradientColors.size() - 1)) {
                        mPosition[i] = 1;
                    } else {
                        mPosition[i] = (1.0f / (mGradientColors.size() - 1)) * i;
                    }
                }
                LinearGradient linearGradient = new LinearGradient(mLineRect.left, 0, mLineRect.right, 0, colors, mPosition, Shader.TileMode.CLAMP);
    ...
    public void setGradientColors(Integer... colors) {
        this.mGradientColors = Arrays.asList(colors);
    }
